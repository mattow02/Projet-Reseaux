...
/* Compare two MAC addresses lexicographically. */
...
/* Find the cost of the link between two devices, or UINT32_MAX if none. */
...
void stp_calculate_paths(stp_network_t* network, const network_config_t* config) {
    ...
    /* Initialize path costs to infinity for non-root switches */
    for (size_t i = 0; i < network->nb_switches; i++) {
    ...
    do {
        ...
        for (size_t i = 0; i < network->nb_switches; i++) {
            ...
            for (size_t j = 0; j < sw->switch_data->nb_ports; j++) {
                /* Find the neighbor switch connected to this port */
...
void stp_elect_root_bridge(stp_network_t* network) {
    ...
    printf("Root bridge: Switch %zu\n", (size_t)(root_bridge - network->switches));
    printf("MAC address: ");
